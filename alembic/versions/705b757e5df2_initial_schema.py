...
def upgrade() -> None:
    ...
    # The room exclusion constraint is NOT created here — revision
    # e0fbfe421403 owns it. It was duplicated in both files, and since the
    # whole upgrade runs in one transaction, the second CREATE aborted the
    # entire chain on a clean database.
...
def downgrade() -> None:
    ...
    # drop_table does NOT drop the types sa.Enum created. Without this the
    # types survive `downgrade base`, and the next `upgrade head` dies on
    # CREATE TYPE ... already exists — i.e. the chain is not re-runnable.
    for enum_name in (
